...
static void dispose_local_variable(int local_variable_count,
                                   ISandBox_LocalVariable *local_variable_list);

static void
dispose_type_derive(ISandBox_TypeSpecifier *type)
{
    int i;
...
static void
dispose_type_specifier(ISandBox_TypeSpecifier *type)
{
...
static void
dispose_code_block(ISandBox_CodeBlock *cb)
{
    int try_idx;
...
static void
dispose_class(ISandBox_Class *cd)
{
    int i;
...
ISandBox_dispose_executable(ISandBox_Executable *exe)
{
    ...
    for (i = 0; i < exe->constant_pool_count; i++) {
        if (exe->constant_pool[i].tag == ISandBox_CONSTANT_STRING) {
